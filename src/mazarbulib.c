...
static void send_line(mazarbulib_t *ctx, char *buf, int n, size_t buf_size) {
  if (n <= 0) {
    return;
  }
  ctx->uart_send(buf,
                 ((size_t)n < buf_size) ? (size_t)n : buf_size - 1);
}

// Writes the "+---...---+---...---+\r\n" border directly to UART.
static void send_separator(mazarbulib_t *ctx) {
...
static bool is_right_aligned(mazarbulib_type_t type) {
  return type == MAZARBULIB_TYPE_INT32  ||
         type == MAZARBULIB_TYPE_UINT32 ||
         type == MAZARBULIB_TYPE_FLOAT  ||
         type == MAZARBULIB_TYPE_DOUBLE ||
...
static void format_value(const mazarbulib_row_t *row,
                         char *buf, size_t buf_len) {
  switch (row->type) {
    case MAZARBULIB_TYPE_INT32:
      snprintf(buf, buf_len, "%" PRId32,
               *(const int32_t *)row->value_ptr);
      break;
    case MAZARBULIB_TYPE_UINT32:
      snprintf(buf, buf_len, "%" PRIu32,
               *(const uint32_t *)row->value_ptr);
      break;
    case MAZARBULIB_TYPE_FLOAT:
      snprintf(buf, buf_len, "%.2f",
               (double)*(const float *)row->value_ptr);
      break;
    case MAZARBULIB_TYPE_DOUBLE:
      snprintf(buf, buf_len, "%.2f",
               *(const double *)row->value_ptr);
      break;
    case MAZARBULIB_TYPE_STRING:
      snprintf(buf, buf_len, "%s",
               (const char *)row->value_ptr);
      break;
    ...
    case MAZARBULIB_TYPE_HEX:
      snprintf(buf, buf_len, "0x%08" PRIX32,
               *(const uint32_t *)row->value_ptr);
      break;
    default:
...
static void render_screen(mazarbulib_t *ctx) {
  char line[MAZARBULIB_LINE_BUF_SIZE_];
  ...
  send_line(ctx, line, n, sizeof(line));
  ...
  for (uint8_t i = 0; i < s->row_count; i++) {
    ...
    format_value(row, val_buf, sizeof(val_buf));

    if (is_right_aligned(row->type)) {
      n = snprintf(line, sizeof(line), "| %-*s | %*s |\r\n",
                   MAZARBULIB_LABEL_WIDTH, row->label,
                   MAZARBULIB_VALUE_WIDTH, val_buf);
    } else {
      n = snprintf(line, sizeof(line), "| %-*s | %-*s |\r\n",
                   MAZARBULIB_LABEL_WIDTH, row->label,
                   MAZARBULIB_VALUE_WIDTH, val_buf);
...
void mazarbulib_prev_screen(mazarbulib_t *ctx) {
  ...
  ctx->active_screen = (ctx->active_screen == 0)
      ? ctx->screen_count - 1u
